...
test('complete Sender to Receiver encrypted text flow', async ({
  browser,
  baseURL,
}) => {
  ...
  await expect(
    sender.getByText('A Receiver supplied the correct PIN'),
  ).toBeVisible();
  await sender.getByRole('button', { name: 'Approve' }).click();
  await expect(
    receiver.getByText('Paired. Secret values stay hidden'),
  ).toBeVisible();
...
test('Relay rejection preserves unsent Sender input', async ({
  browser,
  baseURL,
}) => {
...
test('terminal resume failure clears session credentials', async ({
  page,
  baseURL,
}) => {
  ...
  expect(
    await page.evaluate(() => sessionStorage.getItem('keybridge.room')),
  ).toBeNull();
});
...
test('security headers, transparency, fragment removal, and second Receiver rejection', async ({
  ...
  await expect(
    sender.getByText('no forward secrecy', { exact: false }),
  ).toBeVisible();
  await sender.goto(baseURL!);
  await sender.getByRole('button', { name: 'Create room' }).click();
  ...
  await expect
    .poll(() =>
      pairingQr.evaluate((element) => (element as HTMLCanvasElement).width),
    )
    .toBeGreaterThan(116);
  expect(
    await pairingQr.evaluate((element) => {
      ...
      const pixels = canvas
        .getContext('2d')!
        .getImageData(0, 0, canvas.width, 16).data;
      return Array.from(pixels).every(
  ...
  await expect(malformed.getByRole('alert')).toContainText(
