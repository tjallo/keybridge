...
Protocol version 2 is frozen and archived. Transport protocol version 2 defines the JSON frames between each browser and the Relay. It uses encrypted envelope version 1 without changes. See [protocol version 1](protocol-v1.md) for the archived transport protocol and the envelope key schedule.
