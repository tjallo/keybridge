<script lang="ts">
  ...
  function drawQrCode(): void {
    ...
    canvas.width = canvas.height =
      (qr.getModuleCount() + quietZone * 2) * scale;
...
<section class="room-page">
  <header class="room-header">
    ...
    <div class="room-meta status">
      <span class:connected={state === 'PAIRED'}
    ...
    <div class="pairing-layout">
      <section class="qr-panel">
        ...
        <div class="link-field">
          ...
          <small
            >The link contains the room key. Share the PIN separately.</small
          >
      ...
      <aside class="pin-panel">
        ...
        <p>
          Compare this code through a different channel before you approve the
          Receiver.
        </p>
        {#if state === 'PAIR_PENDING'}
          <div class="approval-card">
            {#if canApprove}
              ...
              <div>
                <button
                  ...
                  onclick={onApprove}>Approve Receiver</button
                ><button
                  class="button ghost"
            ...
            {:else}
              ...
              <p>
                Authentication has not succeeded. Wait for the Receiver to enter
                the PIN.
              </p>
              <button
                class="button ghost"
                onclick={onReject}>Reject request</button
              >
            {/if}
          </div>
        {/if}
  ...
  {:else if state === 'PAIRED'}
    <section class="send-panel">
      ...
      <div class="send-form">
        ...
        <label
          >Expires<select bind:value={ttl}
            ><option value={30}>30 seconds</option><option value={60}
              >1 minute</option
            ><option value={120}>2 minutes</option><option value={300}
              >5 minutes</option
            ></select
          ></label
        >
      ...
      <button
        class="button primary"
        disabled={sending || !label || !value}
        onclick={send}
        >{sending ? 'Encrypting…' : 'Send secret'}
  ...
  <div class="room-actions">
    <button
      class="button ghost"
      onclick={onExtend}>Extend 10 minutes</button
    ><button
      class="button danger"
